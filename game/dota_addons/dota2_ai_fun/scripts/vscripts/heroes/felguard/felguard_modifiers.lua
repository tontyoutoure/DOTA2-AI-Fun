...
function modifier_felguard_overflow:OnCreated()
	...
	ParticleManager:SetParticleControlEnt(self.iParticle1, 1, hParent, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", hParent:GetOrigin(), true)
	...
	ParticleManager:SetParticleControlEnt(self.iParticle2, 1, hParent, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", hParent:GetOrigin(), true)
	...
	ParticleManager:SetParticleControlEnt(self.iParticle3, 1, hParent, PATTACH_ABSORIGIN_FOLLOW, "attach_hitloc", hParent:GetOrigin(), true)
	for i = 1,20 do
...
function modifier_felguard_overflow:OnIntervalThink()
	if IsClient() then return end
	local hParent = self:GetParent()
	if hParent:HasScepter() then
		ApplyDamage({
			...
			damage_flags = DOTA_DAMAGE_FLAG_HPLOSS+DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS+DOTA_DAMAGE_FLAG_NON_LETHAL+DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION+DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION,
			damage = self:GetAbility():GetSpecialValueFor("lifedrain_percentage_scepter")/100*0.1*hParent:GetMaxHealth()
		})
	else
		self:Destroy()
	end

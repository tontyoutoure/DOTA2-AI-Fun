--[[ ============================================================================================================
	...
	Date: March 02, 2015& 13th, April, 2018
...
================================================================================================================= ]]
LinkLuaModifier("modifier_invoker_retro_betrayal", "heroes/hero_invoker/invoker_retro_betrayal.lua", LUA_MODIFIER_MOTION_NONE)

invoker_retro_betrayal=class({})

function invoker_retro_betrayal:OnSpellStart()
	...
	if keys.caster:FindAbilityByName(self:GetAbilityName()) and self == keys.caster:FindAbilityByName(self:GetAbilityName()) then
	...
	if keys.target:HasModifier("modifier_invoker_retro_betrayal") then  --If the unit is already on a unique team, simply refresh the modifier's duration.  This does not trigger the modifier's OnDestroy event.
		keys.caster:AddNewModifier(keys.caster, keys.ability, "modifier_invoker_retro_betrayal", {Duration = iQuasLevel*keys.ability:GetSpecialValueFor("duration_level_quas")*CalculateStatusResist(keys.target)})
		...
		for i=6, 13, 1 do
			if found_new_team == false then
				if unoccupied_teams[tostring(i)] == true then
					...
					keys.target:AddNewModifier(keys.caster, keys.ability, "modifier_invoker_retro_betrayal", {Duration = iQuasLevel*keys.ability:GetSpecialValueFor("duration_level_quas")*CalculateStatusResist(keys.target)})
...
================================================================================================================= ]]
modifier_invoker_retro_betrayal = class({})
function modifier_invoker_retro_betrayal:GetEffectName() return "particles/units/heroes/hero_invoker/invoker_retro_betrayal.vpcf" end
function modifier_invoker_retro_betrayal:GetEffectAttachType() return PATTACH_OVERHEAD_FOLLOW end
function modifier_invoker_retro_betrayal:IsDebuff() return true end
function modifier_invoker_retro_betrayal:IsPurgable() return true end
function modifier_invoker_retro_betrayal:OnDestroy()
	--Remove health labels if no heroes have Betrayal on them anymore.
	if IsClient() then return end
	local keys = {target = self:GetParent()}

...
================================================================================================================= ]]
function invoker_retro_betrayal:OnStolen(hSource)
	if self:GetCaster():HasScepter() and not hSource:GetCaster():HasScepter() then
		self.iQuasLevel = hSource.iQuasLevel+1
		self.iWexLevel = hSource.iWexLevel+1
		self.iExortLevel = hSource.iExortLevel+1
	elseif not self:GetCaster():HasScepter() and hSource:GetCaster():HasScepter() then
		self.iQuasLevel = hSource.iQuasLevel-1
		self.iWexLevel = hSource.iWexLevel-1
		self.iExortLevel = hSource.iExortLevel-1
	else
		self.iQuasLevel = hSource.iQuasLevel
		self.iWexLevel = hSource.iWexLevel
		self.iExortLevel = hSource.iExortLevel	
	end	
end
function invoker_retro_betrayal:OnSpellStart()
	...
	if keys.target:HasModifier("modifier_invoker_retro_betrayal") then  --If the unit is already on a unique team, simply refresh the modifier's duration.  This does not trigger the modifier's OnDestroy event.
		keys.ability:ApplyDataDrivenModifier(keys.caster, keys.target, "modifier_invoker_retro_betrayal", {Duration = keys.ability.iQuasLevel*keys.ability:GetSpecialValueFor("duration_level")})

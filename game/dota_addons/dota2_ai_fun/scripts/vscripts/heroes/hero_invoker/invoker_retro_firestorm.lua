...
function invoker_retro_firestorm_on_spell_start(keys)
	...
	local num_fireballs = iExortLevel*keys.ability:GetSpecialValueFor('num_fireballs_level_exort')
						...
						Timers:CreateTimer({
							...
							callback = function()
								...
								for i, individual_unit in ipairs(nearby_enemy_units) do
									ApplyDamageTestDummy({victim = individual_unit, attacker = keys.caster, damage = keys.FireballLandingDamage, damage_type = DAMAGE_TYPE_MAGICAL,})
								end
								...
								Timers:CreateTimer({
									...
									callback = function()
										...
										for i, individual_unit in ipairs(nearby_enemy_units) do
											ApplyDamageTestDummy({victim = individual_unit, attacker = keys.caster, damage = keys.FireballExplosionDamage, damage_type = DAMAGE_TYPE_MAGICAL,})
										end
...
function modifier_invoker_retro_firestorm_fireball_damage_over_time_aura_on_interval_think(keys)
	ApplyDamageTestDummy({victim = keys.target, attacker = keys.caster, damage = keys.FireballAoEDamagePerSecond * keys.FireballAoEDamageInterval, damage_type = DAMAGE_TYPE_MAGICAL,})
end
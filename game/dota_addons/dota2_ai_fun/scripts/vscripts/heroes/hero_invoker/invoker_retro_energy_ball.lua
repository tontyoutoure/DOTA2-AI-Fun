...
function invoker_retro_energy_ball_on_spell_start(keys)	
	...
	energy_ball_dummy_unit.damage_to_deal = keys.ability:GetSpecialValueFor("damage_level_wex")*iWexLevel
		...
		callback = function()
			...
			if GameRules:GetGameTime() > endTime then  --Explode the energy ball.
				...
				for i, individual_unit in ipairs(nearby_enemy_units) do  --Damage and zap each nearby unit.
					...
					ApplyDamageTestDummy({victim = individual_unit, attacker = keys.caster, damage = energy_ball_dummy_unit.damage_to_deal, damage_type = DAMAGE_TYPE_MAGICAL,})
				end

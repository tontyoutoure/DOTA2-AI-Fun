...
local function HauntInstance(hModifier)
	...
	if hCaster:HasModifier("modifier_avatar_of_vengeance_reality") then
		...
		local hIllusion = CreateUnitByName(hCaster:GetUnitName(), vPos, true, hCaster, hCaster, hCaster:GetTeamNumber())
		hIllusion:SetControllableByPlayer(hCaster:GetPlayerOwnerID(), true)
		
		hIllusion:SetPlayerID(hCaster:GetPlayerID())
		...
		for i=0,23 do
			...
			if hAbility then 
				hIllusion:FindAbilityByName(hAbility:GetAbilityName()):SetLevel(hAbility:GetLevel()) 
			end
		end
		...
		for i=0,9 do
			...
			if hItem then 
				hIllusion:RemoveItem(hIllusion:GetItemInSlot(i))
				hIllusion:AddItem(CreateItem(hItem:GetName(), hIllusion, hIllusion)):SetCurrentCharges(hItem:GetCurrentCharges())
			end
		end
		 
		for i, v in ipairs(tAddModifierList) do
			if hCaster:HasModifier(v) then
				local hModifier = hCaster:FindModifierByName(v) 
		...
		hIllusion:SetForwardVector(vForward)
		hIllusion:SetHealth(hCaster:GetHealth())
		hIllusion:SetMana(hCaster:GetHealth())
		hIllusion:AddNewModifier(hCaster, hAbility, "modifier_illusion", {duration = iStunDuration, outgoing_damage = hModifier.fOutgoing-100, incoming_damage = hModifier.fIncoming-100 })
		hIllusion:MakeIllusion()
		Timers:CreateTimer(0.04, function () hIllusion:MoveToTargetToAttack(hParent) end)
		hIllusion:EmitSound("Hero_Spectre.Haunt")
	end

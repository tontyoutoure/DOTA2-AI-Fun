...
LinkLuaModifier('modifier_persuasive_swindle_lua','heroes/persuasive/persuasive_modifiers.lua', LUA_MODIFIER_MOTION_NONE)
...
function persuasive_swindle_lua:GetCooldown(num)
	if self:GetCaster():HasScepter() then return self:GetSpecialValueFor('cooldown_scepter')
	else return self.BaseClass.GetCooldown(self, num)
	end
end

function persuasive_swindle_lua:GetIntrinsicModifierName()
	return "modifier_persuasive_swindle_lua"
end

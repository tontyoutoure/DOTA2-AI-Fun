...
function intimidator_be_my_friend_lua:GetBehavior()
	...
	if self.hSpecial:GetSpecialValueFor("value") > 0 then
...
function intimidator_be_my_friend_lua:OnSpellStart()
	...
	if self.hSpecial:GetLevel() == 0 and self:GetCursorTarget():TriggerSpellAbsorb( self ) then 
		self:GetCaster():Interrupt()
		return 
	end
	self.tModifiers = self.tModifiers or {}
	self.tParticles = self.tParticles or {}
	if self.hSpecial:GetLevel() > 0 then

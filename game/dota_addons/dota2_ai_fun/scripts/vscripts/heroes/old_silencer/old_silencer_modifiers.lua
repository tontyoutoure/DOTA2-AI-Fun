...
function modifier_old_silencer_silencer:OnAttack(keys)
	...
	if keys.attacker:HasScepter() then
		local tTargets = FindUnitsInRadius(keys.attacker:GetTeamNumber(), keys.attacker:GetOrigin(), nil, keys.attacker:Script_GetAttackRange(), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC+DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NOT_ATTACK_IMMUNE+DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_ANY_ORDER, true)
		for i = 1, #tTargets do
